...
# def stem_string(string):
#     """
#     Returns the string with punctuations removed (no stemming).
    
#     Args:
#         string (str): String to be processed.
    
#     Returns:
#         processed_string (str): String without punctuation.
#     """
#     punc = '''।,;:?!'."-[]{}()–—―~'''
...
#             original_key = key
#             # Stem the key to remove punctuation
#             key = stem_string(key)
#             trgts[original_key] = []  # Initialize the list for the original key
...
#                     hasfound = True
#                     trgts[original_key].append(value)  # Append the original value
...
def filter_dataset(jsonl_file, target_file, lower_bound, upper_bound):
    N = 4
    linecount = 0
    ...
    for line in jsonl_file.iter():
        ...
        for key, values in line.items():
            ...
            for value in values:
                # Calculate the PINC score
                pinc_score = calculate_pinc(key, value, N)

                # Check if the PINC score is within the bounds
                if lower_bound <= pinc_score <= upper_bound:
                    hasfound = True
                    trgts[original_key].append(value)  # Append the original value
...
if __name__ == '__main__':
    # Create the parser
    parser = argparse.ArgumentParser(
        description='Path to the input and output files and the PINC score threshold bounds')
    ...
    parser.add_argument('--lower',
                        metavar='lower',
                        type=float,
                        help='The lower bound of the desired PINC score threshold (0 - 1)')

    parser.add_argument('--upper',
                        metavar='upper',
                        type=float,
                        help='The upper bound of the desired PINC score threshold (0 - 1)')
    ...
    filter_dataset(jsonl_file, target_file, lower_bound, upper_bound)
